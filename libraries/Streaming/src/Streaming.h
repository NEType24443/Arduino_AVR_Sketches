...
#if defined(ARDUINO)
// No stl library, so need trivial version of std::is_signed_ ...
namespace std {
template<typename T>
  struct is_signed_ { static const bool value = false; };
  template<>
  struct is_signed_<int8_t> { static const bool value = true; };
  template<>
  struct is_signed_<int16_t> { static const bool value = true; };
  template<>
  struct is_signed_<int32_t> { static const bool value = true; };
};
#else
#include <type_traits>
#endif
